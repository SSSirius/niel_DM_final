...
var wh=$( window ).height();
         var ww=$( window ).width();
var endbar=barw*1.4;
var endbarleft=barw*0.4;
var endtimebar=barw;
   $("#timebar").width(endtimebar);
  $("#bar").width(endbar);
   // $("#bar").offset({
   //      // left:-endbarleft
   //    });
 ...
 console.log(end);
  ...
  var btnp = endtimebar- $("#timebar").position().left*2.5;
  $("#timebar").draggable({
    // handle: "#timebar",
    axis: "x",
    containment: "parent",
...
// $("#year").width(upw);
       var ofleftyear =  -btnp / barw * (endbar)+0.5*ww;
      $("#year").offset({
        left: ofleftyear
      });
      $("#up").offset({
...
for (var i=0;i < time.length;i++){
      ...
      if ( btnp/ww * 100 > time[i].position){
